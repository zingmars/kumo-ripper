...
path = ".com/p/blog-page.html"
domain = "http://turb0translation.blogspot" # Some of the chapters are hosted on .my tld, which just 301 to the .com one
htmlFileName = "turb0.html"
htmlFile = None
pdfFileName = "turb0.pdf"
...
def ripSite():
	...
	postDiv = soup.find("div", { "class":"post-body" })
	links = postDiv.find_all("a")
	linkArr = []
	for link in links:
	...
	for link in linkArr:
		...
		html = handle.read().decode("UTF-8")
		soup = BeautifulSoup(html, 'html.parser')
		appendToHTML(str(soup.find("h3", { "class":"post-title" })) + "<br />")
		appendToHTML("<div>==============================</div>")
		appendToHTML(str(soup.find("div", { "class":"post-body" })))
		appendToHTML("<div>==============================</div>")
